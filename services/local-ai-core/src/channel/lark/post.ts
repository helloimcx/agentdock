const LARK_CARD_MARKDOWN_TABLE_LIMIT = 5;
...
export function buildLarkPostContent(markdown: string) {
  ...
  return {
    zh_cn: {
      content: [[{
        tag: 'md',
        text: sanitizeLarkPostMarkdown(normalized),
      }]],
    },
  };
}
...
function sanitizeLarkPostMarkdown(markdown: string) {
  return sanitizeLarkMarkdown(markdown);
}

function sanitizeLarkMarkdown(markdown: string) {
