...
export class LocalCoreAcpStore {
  ...
  listAutomationMonitors(workspaceId?: string): AutomationMonitor[] {
    ...
    return rows.map((row) => this.toAutomationMonitor(row));
  }
  ...
  getAutomationMonitor(monitorId: string): AutomationMonitor | undefined {
    ...
    return row ? this.toAutomationMonitor(row) : undefined;
  }
  ...
  createAutomationMonitor(input: AutomationMonitorCreateInput & { platform: NonNullable<AutomationMonitorCreateInput['platform']>; route: NonNullable<AutomationMonitorCreateInput['route']> }): AutomationMonitor {
    ...
    const id = this.createAutomationMonitorId();
    const now = new Date().toISOString();
    this.db.prepare(`
  ...
  updateAutomationMonitor(monitorId: string, input: AutomationMonitorUpdateInput): AutomationMonitor {
    ...
    return this.getAutomationMonitor(monitorId)!;
  }
  ...
  listAutomationMonitorRuns(monitorId: string): AutomationMonitorRun[] {
    ...
    return rows.map((row) => this.toAutomationMonitorRun(row));
  }
  ...
  createAutomationMonitorRun(monitorId: string, status: AutomationMonitorStatus, input: Partial<AutomationMonitorRun> = {}): AutomationMonitorRun {
    ...
    this.updateAutomationMonitorState(monitorId, {
      lastTriggeredAt: triggeredAt,
      lastStatus: normalizedStatus,
      lastError: input.error || '',
    });
    return this.getAutomationMonitorRun(id)!;
  }
  ...
  getAutomationMonitorRun(runId: string): AutomationMonitorRun | undefined {
    ...
    return row ? this.toAutomationMonitorRun(row) : undefined;
  }

  updateAutomationMonitorRun(runId: string, input: Partial<AutomationMonitorRun>): AutomationMonitorRun {
    const existing = this.getAutomationMonitorRun(runId);
    if (!existing) {
    ...
    if (input.status || Object.prototype.hasOwnProperty.call(input, 'error') || input.finishedAt || input.triggeredAt) {
      this.updateAutomationMonitorState(existing.monitorId, {
        lastTriggeredAt: next.triggeredAt,
        lastStatus: next.status,
        lastError: next.error || '',
      });
    }
    return this.getAutomationMonitorRun(runId)!;
  }
  ...
  private toAutomationMonitor(row: LocalAutomationMonitorRow): AutomationMonitor {
  ...
  private toAutomationMonitorRun(row: LocalAutomationMonitorRunRow): AutomationMonitorRun {
    return {
      id: row.id,
  ...
  private createAutomationMonitorId() {
    let id = `monitor:${randomUUID()}`;
    for (let attempt = 0; attempt < 5 && this.getAutomationMonitor(id); attempt += 1) {
      id = `monitor:${randomUUID()}`;
    }
    if (this.getAutomationMonitor(id)) {
