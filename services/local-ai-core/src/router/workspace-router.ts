...
import {
  ...
  buildArtifactMetadataContent,
} from './workspace-artifact-resolver.js';

export { ArtifactContentError };
...
export class WorkspaceRouter {
  ...
  private async prepareAgentMessage(threadId: string, content: string | ChannelInboundMessageContent, workspacePath: string) {
    ...
    const knowledgeBases = selectedIds.size
      ? (await this.options.knowledgeProvider.listKnowledgeBases())
          ...
          .map(({ id, name }) => ({ id, name }))
      : [];
    const pendingHandoff = this.store.sessionHandoffs.getPendingHandoff(threadId);
    ...
    const wrapped = composeAgentMessage(
      displayText,
      knowledgeBases,
      new ManagedSkillCatalog({ workspacePath }),
      undefined,
      pendingHandoff,
    );
    if (pendingHandoff) {

      this.store.sessionHandoffs.markHandoffConsumed(pendingHandoff.id);
    }
  ...
  listThreadHandoffs(threadId: string) {
    return this.store.sessionHandoffs.listHandoffs(threadId);
  }

  getPendingThreadHandoff(threadId: string) {
    return this.store.sessionHandoffs.getPendingHandoff(threadId);
  }
