...
export function registerSecurityHandlers(
  ...
  map.set('workspace-security.update', async (route, req, res) => {
    ...
    json(res, 200, await workspaceRouter.updateWorkspaceSecuritySettings((route as { workspaceId: string }).workspaceId, body as unknown as import('../../../../../packages/contracts/src/index.js').WorkspaceSecuritySettingsUpdateInput));
  });
  ...
  map.set('approvals.list', async (_route, _req, res, url) => {
    ...
    const status = statusParam ? statusParam.split(',').map((item) => item.trim()).filter(Boolean) as import('../../../../../packages/contracts/src/index.js').ApprovalRequestListQuery['status'] : undefined;
  ...
  map.set('approvals.create', async (_route, req, res) => {
    const body = await readJsonBody(req);
    json(res, 200, await workspaceRouter.createApprovalRequest(body as unknown as import('../../../../../packages/contracts/src/index.js').ApprovalRequestCreateInput));
  });
  ...
  map.set('approval.resolve', async (route, req, res) => {
    const body = await readJsonBody(req);
    json(res, 200, await workspaceRouter.resolveApprovalRequest((route as { approvalId: string }).approvalId, body as unknown as import('../../../../../packages/contracts/src/index.js').ApprovalRequestResolveInput));
  });
  map.set('audit-events.list', async (_route, _req, res, url) => {
    const typeParam = url.searchParams.get('type') || '';
    const type = typeParam ? typeParam.split(',').map((item) => item.trim()).filter(Boolean) as import('../../../../../packages/contracts/src/index.js').AuditEventListQuery['type'] : undefined;
