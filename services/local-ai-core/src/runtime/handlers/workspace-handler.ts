...
export function registerWorkspaceHandlers(
  ...
  map.set('workspace-registry.create', async (_route, req, res) => {
    ...
    json(res, 200, await workspaceRouter.createWorkspaceRegistryEntry(body as unknown as import('@cc/superai-contracts').WorkspaceRegistryCreateInput));
  });
  ...
  map.set('workspace-registry.update', async (route, req, res) => {
    const body = await readJsonBody(req);
    json(res, 200, await workspaceRouter.updateWorkspaceRegistryEntry((route as { workspaceId: string }).workspaceId, body as unknown as import('@cc/superai-contracts').WorkspaceRegistryUpdateInput));
  });
