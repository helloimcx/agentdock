...
test('desktop platform normalization maps feishu to lark', () => {
  assert.equal(normalizeDesktopPlatformType('feishu'), 'lark');
  assert.equal(normalizeDesktopPlatformType('lark'), 'lark');
});
...
test('service manager removes lark bindings from generated cc-connect runtime config', async () => {
  const temp = withTempDir();
  try {
    const manager = new ServiceManager(temp.dir);
    const logicalConfig: DesktopConnectConfig = {
      projects: [
        buildProject({
          ...
          platforms: [
            { type: 'feishu', options: { app_id: 'cli_a', app_secret: 'secret_a' } },
            { type: 'telegram', options: { bot_token: 'token_b' } },
          ],
        }),
      ],
    };
    ...
    assert.equal(derived.projects?.[0]?.platforms?.length, 1);
    assert.equal(derived.projects?.[0]?.platforms?.[0]?.type, 'telegram');
  } finally {
    temp.cleanup();
  }
});
