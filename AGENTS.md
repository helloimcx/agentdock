...
## Agent Workflow
Before writing any code, describe the intended approach and wait for approval. If requirements are ambiguous, ask clarifying questions before writing any code. If a user request for code is not aligned with best practices, briefly explain the concern and suggest a better approach. If a task requires changes to more than 3 files, stop and break it into smaller tasks first. When fixing a bug, start by writing a test that reproduces it, then fix the bug until the test passes. After writing code, list relevant edge cases and suggest test cases to cover them. Every time the user corrects the agent, reflect on what went wrong and provide a plan to avoid repeating the same mistake.
