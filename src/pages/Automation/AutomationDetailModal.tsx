    ...
    <>
      <Modal open onClose={onClose} title={automation.title} className="sm:max-w-2xl">
        <div className="space-y-4 text-sm">
          ...
          <section><h3 className="font-medium">Recent runs</h3>{runs.length ? runs.slice(0, 5).map((run) => <div key={run.id} className="flex items-center justify-between gap-2 py-0.5"><p className="truncate">{formatRun(run)} · {run.finishedAt || run.createdAt}</p>{onViewTrace && <Button size="sm" variant="ghost" onClick={() => onViewTrace(run.id)} className="h-6 px-2 text-xs text-primary hover:bg-primary/10 shrink-0"><Activity className="mr-1 h-3 w-3" /> Trace 轨迹</Button>}</div>) : <p>—</p>}</section>
        </div>
      </Modal>
